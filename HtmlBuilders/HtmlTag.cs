    ...
    public IReadOnlyDictionary<string, string> Styles {
      get {
        ...
        /*                var styleRules = styles.Split(';')
                            .Select(styleRule => new {StyleRule = styleRule, SeparatorIndex = styleRule.IndexOf(':')})
                            .Select(a =>
                                    new
                                    {
                                        StyleKey = a.StyleRule.Substring(0, a.SeparatorIndex),
                                        StyleValue = a.StyleRule.Substring(a.SeparatorIndex, a.StyleRule.Length)
                                    });  */
        var styleRulesSplit = styles.Split(';');
        var styleRuleStep1 = styleRulesSplit.Select(styleRule => new { StyleRule = styleRule, SeparatorIndex = styleRule.IndexOf(':') }).ToArray();
