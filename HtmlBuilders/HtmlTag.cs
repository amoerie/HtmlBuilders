...
namespace HtmlBuilders {
  ...
  public class HtmlTag : IHtmlElement, IDictionary<string, string> {
    ...
    /// </summary>
    public IEnumerable<IHtmlElement> Contents {
    ...
    public IEnumerable<HtmlTag> Parents {
      get { return Parent == null ? Enumerable.Empty<HtmlTag>() : new[] {Parent}.Concat(Parent.Parents); }
    }
    ...
    public HtmlTag Data(object data, bool replaceExisting = true) {
      ...
      foreach (var htmlAttribute in htmlAttributes) {
        ...
        Attribute(attribute.StartsWith("data-") ? attribute : "data-" + attribute, Convert.ToString(htmlAttribute.Value),
          replaceExisting);
      }
      return this;
    }
    ...
    public IReadOnlyDictionary<string, string> Styles {
      get {
        ...
        string[][] styleRules = styles.Split(';').Select(styleRule => styleRule.Split(':')).ToArray();
        if (styleRules.All(s => s.Length == 2))
          return styleRules.ToDictionary(styleRule => styleRule[0], styleRule => styleRule[1]);
        IEnumerable<string[]> invalidStyleRules = styleRules.Where(s => s.Length != 2);
        throw new InvalidOperationException(string.Format("Detected invalid style rules: {0}",
          string.Join(",", invalidStyleRules.Select(s => string.Join(":", s)))));
      }
      set {
        if (value.Count == 0) {
          Remove("style");
    ...
    public HtmlTag Class(string @class) {
      ...
      string[] classesToAdd = @class.Split(' ');
    ...
    public HtmlTag RemoveClass(string @class) {
      if (@class == null)
        throw new ArgumentNullException("class");
      string[] classesToRemove = @class.Split(' ');
        ...
        case TagRenderMode.SelfClosing:
          if (Contents.Any()) {
            throw new InvalidOperationException(
              "Cannot render this tag with the self closing TagRenderMode because this tag has inner contents: Count = " +
              Contents.Count());
          }
      ...
      if (htmlDocument.ParseErrors.Any() && validateSyntax) {
        IEnumerable<string> readableErrors =
          htmlDocument.ParseErrors.Select(
            e => string.Format("Code = {0}, SourceText = {1}, Reason = {2}", e.Code, e.SourceText, e.Reason));
    ...
    private bool Equals(HtmlTag other) {
      ...
      if (Count != other.Count)
        return false;
      if (!DictionaryComparer.Equals(this, other, new[] {"class", "style"}))
        return false;
