...
public class HtmlTag : IHtmlElement
{
    ...
    public bool HasAttribute(string attribute) => _attributes.ContainsKey(attribute);
    ...
    /// </summary>
    private HtmlTag WithStyles(IImmutableDictionary<string, string?> styles)
    {
        if (styles.Count == 0)
        {
    ...
    /// </summary>
    private HtmlTag WithClass(IImmutableList<string> classes)
    {
        if (classes.Count == 0)
        {
    ...
    /// <returns>This <see cref="HtmlTag" /></returns>
    public HtmlTag Render(TagRenderMode tagRenderMode) => WithTagRenderMode(tagRenderMode);
    ...
    public static IEnumerable<IHtmlElement> ParseAll(HtmlDocument htmlDocument, bool validateSyntax = false)
    {
        ...
        foreach (var childNode in htmlDocument.DocumentNode.ChildNodes)
        {
            ...
            {
                yield return ParseHtmlText(childNode);
            }
